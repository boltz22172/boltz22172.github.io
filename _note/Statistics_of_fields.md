---
title: "Statistical Physics of Fields"
layout: single
author_profile: false
excerpt: "Notes for the book *Statistical Physics of Fields* (2024 Winter, at Cornell)."
---

<div style="float:right; width:280px; margin:0 0 1rem 1.5rem; border:2px solid #9ca3af; background:#f3f4f6; padding:12px; border-radius:8px; font-size:0.9em; color:#374151;">
  <img src="/images/notes_images/Stat_Field.png" alt="Notes" style="width:100%; border-radius:6px; margin-bottom:10px;">
  <strong>Statistical Physics of Fields</strong><br>
  Statistical field theory reframes statistical physics in the language of fields: coarse-graining turns microscopic degrees of freedom into collective variables, and fluctuations become the central actor. This perspective links models to universal behavior through correlation functions and the renormalization group.<br><br>
  <em>Book notes; may contain mistakes.</em>
</div>

These are my self-study notes written during the winter of 2024, while I was at Cornell, based on  Prof. Kardar’s  
[*Statistical Physics of Fields*](https://www.cambridge.org/core/books/statistical-physics-of-fields/06F49D11030FB3108683F413269DE945).

The book adopts a distinctly modern perspective and is particularly well suited for a second pass through statistical mechanics. At the same time, it is not always easy to read, and some derivations are nontrivial for beginners. I hope these notes may be of some help.

## Notes (PDF)

- [Statistical Fields: A Phenomenological Introduction](/files/notes/Stat_Field.pdf)


This page is still under construction. Stay tuned.


<div style="clear:both;"></div>
